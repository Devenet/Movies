...
abstract class Path {
	...
	static function movie($id) {
		...
		$title = isset($movies[$id]) ? '/'.String::slug($movies[$id]['title']) : '';
...
function moviePage() {
	...
	$tpl->assign('social', [
		...
		'twitter' => urlencode(($movie['status']==Movie::SEEN ? 'I’ve rated '.($movie['note']/2).' '.String::pluralize('star', $movie['note']/2) : 'I want to see').' “'.$movie['title'].'” '.$social_url),
		'url' => $social_url
	]);
	$tpl->draw('movie');
	exit();
